...
#
# Writes: dist/wipeoutxl-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix wipeoutxl \
  --exe-name Wipeout2K97XL \
  --display-name "Wipeout2K97XL" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Wipeout2K97XL disc" \
  ...
  --project-file PUBLICATION.md \
  --project-file VERIFICATION.md \
  --project-file THIRD_PARTY.md \
  --project-file RELEASE_NOTES.md \
  --project-file keybinds.ini \
  --project-file input.ini \
  --project-file gamecontrollerdb.txt \
  --project-file tools/stage_input_defaults.cmake \
  --project-dir seeds \
  --project-dir docs \
